...
public class WebSocketConnection implements WebSocket {
   ...
   public static boolean reconnectionHappening = false;
   ...
   /* (non-Javadoc)
    * @see de.tavendo.autobahn.Wamp#isConnectionClosed()
    */
   public boolean isConnectionClosed() {
   	// TODO Auto-generated method stub
   	return reconnectionHappening;
   }
   ...
   public void failConnection(int code, String reason) {
      ...
      if (mReader != null) {
         mReader.quit();
         try {
            // if(Thread.holdsLock(mReader))
            // mReader.wait();
             if(!mReader.mSocket.isBlocking()) {
                 mReader.join();
             }
             else{
                 mReader.mSocket.close();
                 mReader.join();
             }
      ...
      if (mWriter != null) {
         ...
         try {
             if(!mWriter.mSocket.isBlocking()) {
                 mWriterThread.join();
             }
             else{
                 mWriter.mSocket.close();
                 mWriterThread.join();
             }
         } catch (InterruptedException  | IOException e) {
            if (DEBUG) e.printStackTrace();
   ...
   public void connect(String wsUri, String[] wsSubprotocols, ConnectionHandler wsHandler, WebSocketOptions options, List<ContentValues> headers) throws WebSocketException {
